<template>
  <div>
    ...
    <project-compound-form
      :model-value="dialog"
      :is-edit="isEdit"
      :compound-key-item="selectedItem"
...
import ProjectCompoundForm from './ProjectCompoundForm.vue'
...
export default {
  ...
  data() {
    return {
      ...
      tableColumns: [
        ...
        {
          key: 'modType', label: 'Mod Type', sortable: true, searchable: true,
        },
  ...
  computed: {
    items() {
      return this.$store.getters['project/getCompoundKeyOptions']?.(this.compoundKeyName) || []
    },
    filteredItems() {
  ...
  methods: {
    ...
    deleteItem(item) {
      ...
      if (originalIndex !== undefined && originalIndex >= 0) {
        ...
        this.$store.dispatch('project/updateCompoundKeyOptions', {
          compoundKeyName: this.compoundKeyName,
          keyItems: newItems,
        })
      }
    },

    // 1. Fix the handleSave method in your table component
    handleSave(item) {
      this.dialog = false
