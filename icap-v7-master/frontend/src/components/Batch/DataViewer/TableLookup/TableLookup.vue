...
export default {
  ...
  computed: {
    ...
    testTable() {
      ...
      const { table_unique_id } = definitionTables.find(table => table.table_name === this.selectedTableName)

      // eslint-disable-next-line camelcase
      return this.tables.find(i => i.table_unique_id === table_unique_id)
    },
    items() {
      let items = this.testTable?.rows || []
    ...
    extractedColumns() {
      const rows = this.items // Get filtered rows
      const columnMap = {}
      // Loop over all queries
      this.queries.forEach(query => {
        const ruleItems = query?.group?.items || []

        ruleItems.forEach(rule => {
          if (rule?.data?.valueType !== 'column') return

          // Extract target column name from <C>value</C>
          const match = rule.data.value?.match(/<C>(.*?)<\/C>/)
          if (!match || !match[1]) return

          const targetColumn = match[1]

          rows.forEach(row => {
            const value = row[targetColumn]?.v

            if (value !== undefined && value !== null) {
              if (!columnMap[targetColumn]) {
                columnMap[targetColumn] = []
              }
              columnMap[targetColumn].push(value) // Allow duplicates
            }
          })
        })
      })
      ...
      return Object.entries(columnMap).map(([label, value]) => ({
        label,
        value,
      }))
    },
