...
export default {
  ...
  methods: {
    ...
    parseRule(rule) {
      ...
      const lowerColumn = `LOWER(${rule.column})`

      if (rule.valueType === 'column') {
        const lowerValue = `LOWER(${ruleValue})`

        switch (rule.operator) {
          case 'ILIKE':
            ruleString = `<P>${lowerColumn} LIKE ${lowerValue}</P>`
            break
          case 'FUZZY MATCH':
            ruleString = `<P>UTL_MATCH.JARO_WINKLER_SIMILARITY(${lowerColumn}, ${lowerValue}) > 93</P>`
            break
          default:
            ruleString = `${column} IN ${ruleValue}`
        }
      } else {
        const quotedValue = `'${ruleValue}'`
        const lowerValue = `LOWER(${quotedValue})`

        switch (rule.operator) {
          case 'ILIKE':
            ruleString = `${lowerColumn} LIKE ${lowerValue}`
            break
          case 'FUZZY MATCH':
            ruleString = `UTL_MATCH.JARO_WINKLER_SIMILARITY(${lowerColumn}, ${lowerValue}) > 93`
            break
          default:
            ruleString = `${column} = ${quotedValue}`
        }
      }
