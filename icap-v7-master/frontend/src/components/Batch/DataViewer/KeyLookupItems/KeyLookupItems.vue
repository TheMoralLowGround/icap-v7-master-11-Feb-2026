...
export default {
  ...
  computed: {
    records: {
      get() {
        return this.$store.getters['dataView/keyLookupItems']
      },
      set(value) {
        this.$store.commit('dataView/SET_KEY_LOOKUP_ITEMS', value)
      },
    },
    dragOptions() {
      return {
  ...
  methods: {
    //  Dispatches actions to set the selected record for editing and switches the mode to 'key-lookup'.
    editRecord(index) {
      this.$store.dispatch('dataView/setKeyLookupItemByIndex', index)
      this.$store.dispatch('dataView/setMode', 'key-lookup')
    },
    // Removes the record from the list of key lookup items.
    deleteRecord(index) {
