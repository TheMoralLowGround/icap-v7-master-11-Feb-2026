<!--
 ...
 Key Data Properties:
   ...
   - `removeFromDisk`: A boolean that determines whether batch files will also be deleted from disk.
  ...
  >
    <b-card-text>
      <div v-if="ids.length === 1">
        Are you sure you want to delete the batch <span class="text-primary">{{ ids[0] }}</span>?
      </div>
      <div v-else>
      ...
      <div class="mt-1">
        <b-form-checkbox
          v-model="removeFromDisk"
        >
          Also delete batch files from disk
        </b-form-checkbox>
      </div>
...
import {
  BModal, BCardText, BButton, BSpinner, BFormCheckbox,
} from 'bootstrap-vue'
...
export default {
  ...
  methods: {
    // Confirm handler for delete batch
    confirmHandler(event) {
      ...
      let endpoint = '/batches/delete_multiple/'
