...
export default {
  ...
  methods: {
    async submitHandler(bvModalEvt) {
      ...
      try {
        // POST request with multipart/form-data using axios - same pattern as dictionaries
        const response = await axios.post(
          '/pipeline/qdrant_vector_db/',
          formData,
          {
            params: {
              endpoint: `parties/processes/${this.processUid}/tables/${this.tableName}/import`,
              request_type: 'POST',
            },
            headers: {
        ...
        if (response.data && response.data.warnings && response.data.warnings.length > 0) {
          const warningMessage = response.data.warnings.join('\n')
          this.$toast({
            component: ToastificationContent,
            props: {
