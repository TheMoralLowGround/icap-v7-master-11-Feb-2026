<template>
  <li
    ...
    v-on="$listeners"
  >
    <!-- Show default point if no icon and not robot -->
    <div
      v-if="!icon && !isAgent"
      class="timeline-item-point"
    />

    <!-- Show regular icon inside the circle -->
    <div
      v-else-if="icon && !isAgent"
      class="timeline-item-icon d-flex align-items-center justify-content-center rounded-circle"
    >
      <feather-icon :icon="icon" />
    </div>

    <!-- Show robot icon if isAgent is true -->
    <div
      v-else-if="isAgent"
      class="timeline-item-icon d-flex align-items-center justify-content-center rounded-circle"
...
<script>
import RobotIcon from '../../../components/AiAgent/RobotIcon.vue'
...
.timeline-item {
  ...
  .timeline-item-icon {
    position: absolute;
    left: -12px; // Same positioning for all icons
    top: 0;
    width: 24px;
    height: 24px;
