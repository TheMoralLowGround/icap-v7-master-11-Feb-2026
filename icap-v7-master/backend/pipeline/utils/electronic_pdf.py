...
from utils.classification_utils import get_project_by_profile
from pipeline.utils.process_batch_utils import get_new_email_batch_id

# ############# For testing ###############
# def get_project_by_profile(profile_name):
#     return "test_profile"

# def get_new_email_batch_id():
#     return "test_batch_id"

# #########################################

def decode_private_use_chars(text):
    """
    Decode private use area characters to readable ASCII.
    Many PDFs use U+F000-U+F0FF range to encode standard ASCII characters.
    Common pattern: U+F020-U+F07E maps to ASCII 0x20-0x7E (space through ~)
    """
    if not text:
        return text
    ...
    for char in text:
        code = ord(char)
        # Check if it's in the private use area commonly used for ASCII mapping
        if 0xF020 <= code <= 0xF0FF:
            # Map U+F0XX to 0x00XX (standard ASCII/Latin)
            ascii_code = code - 0xF000
            if 0x20 <= ascii_code <= 0x7E or 0xA0 <= ascii_code <= 0xFF:
                decoded.append(chr(ascii_code))
            else:
                # Keep original if mapping doesn't make sense
                decoded.append(char)
        else:
            # Keep non-private-use characters as-is
            decoded.append(char)
...
def process_word(chars, span_info, page_num, page_height, scale_factor, output_data, pixel_block_bbox):
    ...
    # The text should already be decoded by the caller, but keep as fallback
    # Note: Don't filter out private use chars here - they should be decoded first
    cleaned_text = word_text
    
    # Skip if word is empty or just spaces after cleaning
...
def extract_words_with_pymupdf(pdf_path, dpi=300):
    """Extract words using PyMuPDF (fitz) which handles custom font encodings better"""
    ...
    for page_num in range(len(doc)):
        ...
        # Process blocks from PyMuPDF
        for block in text_dict.get("blocks", []):
            if block.get("type") == 0:  # Text block
                block_bbox = block.get("bbox", [0, 0, 0, 0])
                pixel_block_bbox = [
                    block_bbox[0] * scale_factor,
                    block_bbox[1] * scale_factor,
                    block_bbox[2] * scale_factor,
                    block_bbox[3] * scale_factor
                ]
                
                # Process lines in the block
                for line in block.get("lines", []):
                    # Process spans (text segments with same formatting)
                    for span in line.get("spans", []):
                        text = span.get("text", "").strip()
                        if not text:
                            continue
                        
                        # Decode private use area characters to readable text
                        decoded_text = decode_private_use_chars(text)
                        ...
                        font_info = {
                            'font': font_name.lower().replace('+', '').split('-')[0] if font_name else 'arial',
                            'family': 'swiss',
                            'size': font_size,
                            'bold': bool(font_flags & 2**4),  # Bold flag
                            'italic': bool(font_flags & 2**1),  # Italic flag
                            'underline': False
                        }
                        
                        # Determine font family
                        if any(serif in font_name.lower() for serif in ['times', 'georgia', 'garamond', 'serif']):
                            font_info['family'] = 'serif'
                        elif any(mono in font_name.lower() for mono in ['courier', 'consolas', 'mono']):
                            font_info['family'] = 'monospace'
                        
                        # Convert bbox to pixels
                        pixel_x0 = span_bbox[0] * scale_factor
                        pixel_y_top = span_bbox[1] * scale_factor
                        pixel_x1 = span_bbox[2] * scale_factor
                        pixel_y_bottom = span_bbox[3] * scale_factor
        ...
        for page_num, page in enumerate(pdf.pages):
            ...
            for block in blocks:
                ...
                for word_info in block['words']:
                    ...
                    # Decode private use area characters before processing
                    decoded_text = decode_private_use_chars(word_info['text'])
                    
                    # Skip if empty after decoding
                    if not decoded_text.strip():
                        continue
                    
                    # Create a char-like structure for the word with decoded text
                    word_char = [{
                        'text': decoded_text,
                        'x0': word_info['x0'],
                        'top': word_info['top'],
...
def process_files(file_paths, output_folder, batch_id, profile_name, doc_type="", dpi=300):
    ...
    vendor = ""
    project = get_project_by_profile(profile_name)
    language = "English"
    ...
    for doc_index, pdf_path in enumerate(renamed_pdfs.keys(), 1):
        try:
            ...
            extracted_data, page_dims = extract_words_with_pymupdf(pdf_path, dpi=dpi)
            print(f"Extracted {len(extracted_data)} words from {len(page_dims)} pages")
            
            # Convert PDF to images
            pages = convert_from_path(pdf_path, dpi=dpi)
            print(f"Converted {len(pages)} pages from PDF to image")
            
            # Process each page and save as TIF
            tif_files = {}
            for i, page in enumerate(pages):
                # Save TIFF file with current tm_no
                ...
                print(f"Saved TIFF file: {tif_filename} (Page {i+1} of {len(pages)})")
                
                # Increment tm_no for next page
                tm_no += 1
...
def process_folder(input_folder, output_folder, batch_id, doc_type="", dpi=300):
    """Process all PDF files from a folder and combine them into a single RAJson output"""
    electronic_pdf_script_version = "v6.14042025.0-ElectronicPdf"
    print("ElectronicPdfScriptVersion", electronic_pdf_script_version)
    vendor = ""
    project = ""
    ...
    # Use the process_files function to process the PDF files
    return process_files(pdf_files, output_folder, batch_id, doc_type, dpi)


# if __name__ == "__main__":
#     pdf_files = [
#         r"D:\aidb\RAjson\Rajsonv6\inputs\Commercial Invoice23131.pdf", 
#         r"D:\aidb\RAjson\Rajsonv6\inputs\Commercial Invoice23664.pdf"
#     ]
    
#     # Base output folder
#     output_folder = r"D:\aidb\RAjson\Rajsonv6\outputv61"
    
#     # Generate batch ID 
#     batch_id = generate_batch_id(output_folder)
    
#     doc_type = ""
    
#     # Process the PDF files directly
#     process_files(pdf_files, output_folder, batch_id, doc_type)

def convert_ra_json_external(extracted_data,page_dims,batch_id,requested_by="EDM2.0"):
    ra_json = {
        "id": "", 
        "TYPE": "AIDBSERVICE",
        "bvOCR": "S",
    ...
    for page_num, words in sorted(pages_group.items()):
        ...
        if page_num == 1:
            page_node["DATAFILE"] = f"{page_num}_layout.xml"
            page_node["OCRSLang"] = "0"
        # Process blocks
        blocks = group_words_by_block(words)
        block_index = 1
...
def extract_ra_json_from_pdf(pdf_path):
    dpi = 300
    print("Extract RA JSON from pdf triggered.")
    ...
    doc.close()
    print(f"PDF contains {num_pages} pages")
    # Extract text data using PyMuPDF (better handling of custom fonts)
    print(f"Extracting text from PDF: {pdf_path}")
    extracted_data, page_dims = extract_words_with_pymupdf(pdf_path, dpi=dpi)
    print(f"Extracted {len(extracted_data)} words from {len(page_dims)} pages")
    # Convert to RA JSON format for this document
    ra_json = convert_ra_json_external(extracted_data,page_dims,batch_id)
    # We only take the first document node
    if ra_json["nodes"] and len(ra_json["nodes"]) > 0:
        document_node = ra_json["nodes"][0]
        document_id = f"{batch_id}.{0:02d}"
        # Update document node properties
        document_node["id"] = document_id
        document_node["Vendor"] = None
        document_node["DocType"] = None
        document_node["Project"] = None
        document_node["Language"] = None
        document_node["DefinitionID"] = None
        document_node["NameMatchingText"] = None
        document_node["layout_id"] = None
        document_node["DocumentType"] = None
        
        # Add this document node to the combined result
        ra_json["nodes"] = document_node
    return ra_json
...
#     # else:
#     #     print("Usage: python electric_pdf.py <pdf_path>")
#     pdf_path = "DHL_SLInz_32707.pdf"
#     result = extract_ra_json_from_pdf(pdf_path)
