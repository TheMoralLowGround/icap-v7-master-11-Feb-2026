            ...
            for idx, path_to_xml_or_page in enumerate(xml_list_or_ra_json_pages):
                ...
                for k, v in chunk_page_data[0].items():
                    for item_idx, item in enumerate(v):
                        ...
                        if chunk_data in value_temp and f"{item[0]}_{item[1]}" not in value_position_mapped_done_list:                             
                ...
                for k, v in chunk_page_data[0].items():
                    for item_idx, item in enumerate(v):
                        ...
                        if fuzz.ratio(item[0], kv['value']) >= 90:
                            ...
                            kvv_items.append(f"{item[0]}{item[1]}{page_id}")
                            break
                        elif kv['key'] in item[0] and kv['value'] in item[0]:
...
def add_postion_information_to_kvv_new(xml_list,ra_json,is_ra_json_available,data_json, kvv_items = []):
    ...
    for values in  kv_list:
                ...
                for k, v in chunk_page_data[0].items():
                    for item_idx, item in enumerate(v):
                        ...
                        if fuzz.ratio(item[0].strip(), values["value"].strip()) >= 90: 
                            ...
                            break
                        elif values["value"] in item[0] and values["key"] in item[0]:
                            l,t,r,b = item[1].split(',')
                            char_threshold = (int(r) - int(l))/ len(item[0])
                            ...
                            if comined_cell_done_dict.get(f"{item[0]}{item[1]}{page_id}",None) == None:
                                comined_cell_done_dict[f"{item[0]}{item[1]}{page_id}"] = item[0].replace(values['key'],"").replace(values['value'],"")
                            else:
        ...
        if values["value"] == "":
            ...
            values["vaule_position"] = "0,0,0,0"
        else:
            values["page_id"] = page_id_final
            values["value_position"] = cell_value_position
    return data_json
