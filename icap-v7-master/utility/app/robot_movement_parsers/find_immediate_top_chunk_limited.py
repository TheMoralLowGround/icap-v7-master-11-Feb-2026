from . import get_bottom_pos_str, get_left_pos_str, get_right_pos_str


def process(point_left, point_right, point_top, data, point_page_id, limit_threshold):
    chunk_list = []
            ...
            for top, line_chunks in value.items():
                if int(top) < point_top:
                    for chunk in line_chunks:
                        ...
                        chunk_bottom = get_bottom_pos_str.process(chunk[1])
                        chunk_right = get_right_pos_str.process(chunk[1])
                        if chunk_bottom < point_top:
                            if (abs(point_right - chunk_right) < limit_threshold) and (
                                abs(point_left - chunk_left) < limit_threshold
                            ):
                                chunk_list.append(
