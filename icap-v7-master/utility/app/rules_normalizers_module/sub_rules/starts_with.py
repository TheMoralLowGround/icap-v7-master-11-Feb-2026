...
def apply_starts_with_function(rule_inputs, input_text):
    try:
        ...
        if string_manipulation_type == "string":
            if output_text.lower().startswith(manipulative_value.lower()):
                return True
        elif string_manipulation_type == "shape":
            if output_shape.startswith(manipulative_shape):
                return True
        else:
