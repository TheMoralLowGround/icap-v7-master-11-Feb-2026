...
from app import table_keys
from app import utility_central
from app import post_processing
from app.output_module import output_central
from app.table_api_excel import process_excel_table
from app.table_keys_excel import excel_table_keys
...
def do_work(ch, method, properties, body):
    ...
    try:
        if message_type == 'process_table_keys':
            func_timeout(WORKER_TIMEOUT_SECONDS, table_keys.process_table_keys, args=(data,))
        elif message_type == 'excel_table_process':
            func_timeout(WORKER_TIMEOUT_SECONDS, process_excel_table, args=(data,))
        elif message_type == 'excel_table_keys_process':
            func_timeout(WORKER_TIMEOUT_SECONDS, excel_table_keys, args=(data,))
        elif message_type == 'keyval_extractor':
            func_timeout(WORKER_TIMEOUT_SECONDS, utility_central.UtilityCentral, args=(data,))
        elif message_type == 'post_processing':
            func_timeout(WORKER_TIMEOUT_SECONDS, post_processing.post_processing, args=(data,))
        elif message_type == 'output_json':
            func_timeout(WORKER_TIMEOUT_SECONDS, output_central.output_json, args=(data,))
...
def main():
    ...
    props = {'connection_name' : 'utility_consumer'}
    ...
    channel.basic_consume(queue='to_utility', on_message_callback=callback)
