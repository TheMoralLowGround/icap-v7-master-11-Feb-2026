...
```
docker build . -f Dockerfile -t classifier_web
docker build ./nginx -f ./nginx/Dockerfile --build-arg CONFIG_FILE=nginx.prod.conf.template -t classifier_nginx
docker build ./postgres -f ./postgres/Dockerfile -t classifier_postgres
```
