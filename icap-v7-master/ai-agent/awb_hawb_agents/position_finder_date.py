...
def is_valid_date(value):
    date_pattern = r'\b(\d{1,2}[\s/-]?[A-Za-z]{3,9}[\s/-]?\d{2,4}|\d{1,2}[\s/-]?\d{1,2}[\s/-]?\d{2,4}|\d{4}[\s/-]?\d{1,2}[\s/-]?\d{1,2})\b'
    ...
    if re.match(r'^\d{1,2}[A-Za-z]{3,9}\d{2,4}$', date_str):
        date_str = re.sub(r'^(\d{1,2})([A-Za-z]{3,9})(\d{2,4})$', r'\1 \2 \3', date_str)
...
def find_position_info(ra_json,date_data):

    position_data = {"batch_id":"","document_id":"","page_id":"","position":"0,0,0,0"}
    position_data["batch_id"] = ra_json["id"]

    all_position_data_list = []
    
    for doc_idx, doc_wise_ra_json in enumerate(ra_json["nodes"]):
        for idx, page in enumerate(doc_wise_ra_json["children"]):
           
            page_id = page["id"]
            
         
            chunk_page_data = get_ra_json_to_text_info_list(page)
            
        
            for k, v in chunk_page_data[0].items():
                for item_idx, item in enumerate(v):
                   
                    is_valid, date_data_from_ra_json = is_valid_date(item[0].strip())
                    if is_valid:
                        print(date_data_from_ra_json)
                    
                    if is_valid and fuzz.ratio(date_data_from_ra_json, date_data) >= 95:

                        position_data["position"] = item[1]
                        position_data["document_id"] = doc_wise_ra_json["id"]
                        position_data["page_id"] = page_id
                        #return position_data
                        
                        all_position_data_list.append(copy.deepcopy(position_data))
                   
    if all_position_data_list != []:
        return all_position_data_list[-1]
    position_data["document_id"] = ra_json["nodes"][0]["id"]
    position_data["page_id"] = ra_json["nodes"][0]["children"][0]["id"]
    return position_data